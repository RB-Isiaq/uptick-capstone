...
import { startupInput, subText1, subText2, title } from './constants';
...
const StartupForm = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setIsLoading(true);
      const result = await postData(``, formObject);
      ...
      if (error instanceof Error) {
        ...
        setMessage('Application is currently unavailable');
        setIsModalOpen((prev) => !prev);
      }
    } finally {
      setIsLoading(false);
  ...
  return (
    <section className="mt-[-120px] pt-[120px] ">
      ...
      <div className="w-full flex flex-col justify-between gap-5   px-3 py-[60px] lg:py-[80px] md:px-10 xl:px-[170px]  max-w-[1440px] mx-auto font-raleway bg-[#fff]">
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-[575px]">
          <h1 className="text-black text-[18px] md:text-[24px] mb-[10px] md:mb-[20px] leading-[160%] lg:mt-5">
            SUBMIT YOUR APPLICATION
          </h1>
          {startupInput.map((inp) => (
            <div
              key={inp.id}
              className="flex flex-col w-full justify-between gap-1 mb-6"
            >
              <label
                htmlFor={inp.name}
