  ...
  return (
    <section className=" bg-[#EDF2FF] py-[50px]" id="why">
      ...
      >
        ...
        <div className="w-full flex flex-col lg:flex-row flex-wrap justify-between items-center gap-3">
          {reasons.map((reason) => (
            <div
              key={reason.id}
              className="w-[311px] h-[309px] flex flex-col  gap-[15px] items-start p-[30px] border border-[#4D4D4D] rounded-2xl bg-[#1C1C1C] hover:shadow-lg  transition-all"
            >
              <Image src={reason.icon} alt={reason.title} />
              <h1 className="w-[232px] font-raleway  text-[24px] leading-[38.4px] text-[#DAE5FF] font-medium">
                {reason.title}
              </h1>
